...
export default async function PublicCheckInPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <main className="public-checkin-shell" lang="en"><section className="page-heading public-checkin"><div className="property-header"><Image className="property-image" src="/images/quiet-2br-old-town-apartment.jpg" width={1600} height={1068} priority alt="Kitchen and living area of Quiet 2BR Old Town Apartment"/><div className="property-title"><span className="eyebrow">Havelská · check-in</span><strong>Quiet 2BR Old Town Apartment</strong></div></div><h1>{template.title}</h1><p>{template.introduction}</p><CheckInForm token={token} checkIn={result.stay.checkIn} checkOut={result.stay.checkOut} template={template}/></section></main>;
}
