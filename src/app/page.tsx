...
export default async function Dashboard() {
  ...
  return <>
    ...
    <section className="dashboard-calendar" aria-labelledby="dashboard-calendar-title">
      ...
      <div className="mobile-calendar-agenda">{Array.from({ length: 28 }, (_, index) => { const day = addDays(start, index); const matches = staysOn(stays, day); return <div className={`agenda-day${matches.length ? " has-stay" : ""}`} key={`agenda-${iso(day)}`}><time>{weekday.format(day)} · {shortDate.format(day)}</time>{matches.length ? matches.map((stay) => <Link href={`/pobyty/${stay.id}`} key={stay.id}>{stay.source === "airbnb" ? "Airbnb" : "Pobyt"} · {stay.guests} hosté</Link>) : <span>Volno</span>}</div>; })}</div>
