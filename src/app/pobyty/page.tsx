...
function StayList({ stays, editable = false }: { stays: Stay[]; editable?: boolean }) {
  return <div className={`stay-list${editable ? "" : " archived-stay-list"}`}>{stays.map((stay) => { const guestCountConfirmed = stay.source === "manual" || Boolean(stay.guestCountManuallySet); return <article className="stay-card" key={stay.id}><div className="date-tile"><strong>{fromIso(stay.checkIn).getDate()}</strong><span>{new Intl.DateTimeFormat("cs-CZ", { month: "short" }).format(fromIso(stay.checkIn))}</span></div><div><span className="eyebrow">{stay.source === "manual" ? "Ručně" : "Airbnb"}</span><h2><Link href={`/pobyty/${stay.id}`}>{detail.format(fromIso(stay.checkIn))} – {detail.format(fromIso(stay.checkOut))}</Link></h2><p>{stay.guests} hosté · připravit pro {stay.preparationGuests}{stay.note ? ` · ${stay.note}` : ""}</p><span className={`guest-count-status${guestCountConfirmed ? " confirmed" : ""}`}>{guestCountConfirmed ? "✓ Počet hostů potvrzen" : "! Ověřit počet hostů"}</span><div className="stay-card-actions"><Link className="text-link" href={`/pobyty/${stay.id}`}>{editable ? "Otevřít provoz pobytu →" : "Otevřít detail pobytu →"}</Link>{editable && <form action={updateGuests} className="guest-count-form" key={`${stay.id}-${stay.guests}`}><input type="hidden" name="stayId" value={stay.id}/><label><span>Hosté</span><select name="guests" defaultValue={stay.guests}>{[1, 2, 3, 4].map((count) => <option key={count} value={count}>{count}</option>)}</select></label><button className="button secondary" type="submit">{guestCountConfirmed ? "Potvrzeno ✓" : "Potvrdit"}</button></form>}</div></div></article>; })}</div>;
}
