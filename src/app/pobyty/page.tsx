...
export default async function StaysPage() {
  ...
  return <>
    ...
    <section className="calendar-section" aria-labelledby="calendar-title"><div className="calendar-title"><div><span className="eyebrow">Přehled rezervací</span><h2 id="calendar-title">Týdenní kalendář</h2></div><p>Den odjezdu je opět volný.</p></div><div className="calendar-scroll"><div className="week-calendar"><div className="calendar-corner">Týden</div>{Array.from({ length: 7 }, (_, index) => <div className="calendar-weekday" key={index}>{weekdayFormat.format(addDays(start, index))}</div>)}{weeks.flatMap((weekStart) => [<div className="calendar-week-label" key={`${iso(weekStart)}-label`}>{dayFormat.format(weekStart)} – {dayFormat.format(addDays(weekStart, 6))}</div>, ...Array.from({ length: 7 }, (_, index) => { const day = addDays(weekStart, index); const dayStays = staysOnDay(stays, day); return <div className={`calendar-day${iso(day) === iso(today) ? " today" : ""}`} key={iso(day)}><time dateTime={iso(day)}>{dayFormat.format(day)}</time>{dayStays.map((stay) => <Link className={`calendar-stay ${stay.source}`} href={`/pobyty/${stay.id}/upravit`} key={stay.id} title={`${stay.source === "airbnb" ? "Airbnb rezervace" : "Ruční pobyt"}: ${stay.guests} hosté, připravit pro ${stay.preparationGuests}`}><strong>{stay.source === "airbnb" ? "Airbnb" : "Ruční pobyt"}</strong><span>{stay.preparationGuests} osoby</span></Link>)}</div>; })])}</div></div></section>
