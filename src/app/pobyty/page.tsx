...
export default async function StaysPage({ searchParams }: StaysPageProps) {
  ...
  const first = stays.find((stay) => stay.checkOut >= iso(today));
  ...
  const agendaStart = monday(first ? fromIso(first.checkIn) : today);
  ...
  return <><section className="page-heading split"><div><span className="eyebrow">Plán provozu</span><h1>Pobyty</h1><p>Každý pobyt má vlastní detail s úkoly, klíči, zprávami a hlášením hostů.</p></div><div className="header-actions"><form action={syncAirbnbCalendar}><button className="button secondary" type="submit">Synchronizovat Airbnb</button></form><Link className="button secondary" href="/export">Export CSV</Link><Link className="button" href="/pobyty/novy"><PlusIcon/>Přidat</Link></div></section>
    <section className="calendar-section month-calendar-section"><div className="month-calendar-title"><div><span className="eyebrow">Přehled rezervací</span><h2>{monthLabel.format(displayMonth)}</h2></div><div className="month-navigation"><Link href={`/pobyty?month=${monthKey(previousMonth)}`}>← Předchozí</Link><Link href={`/pobyty?month=${monthKey(today)}`}>Dnes</Link><Link href={`/pobyty?month=${monthKey(nextMonth)}`}>Další →</Link></div></div><p className="calendar-explainer">Pruh označuje noclehy. Den odjezdu je vždy samostatně jako „odjezd ráno“.</p>
...
function MonthWeek({ weekStart, displayMonth, stays, today }: { weekStart: Date; displayMonth: Date; stays: Stay[]; today: Date }) {
  ...
  const laidOut = bars.map((stay) => {
    ...
    laneEnds[lane] = end;
    const startColumn = Math.round((fromIso(start).getTime() - weekStart.getTime()) / DAY) + 1;
    const endColumn = Math.round((fromIso(end).getTime() - weekStart.getTime()) / DAY) + 1;
    return { stay, lane, startColumn, endColumn };
  });
  const lanes = Math.max(1, laneEnds.length);
  return <div className="month-week" style={{ gridTemplateRows: `108px repeat(${lanes}, 32px)` }}>{days.map((day) => { const departures = departuresOn(stays, day); const outside = day.getMonth() !== displayMonth.getMonth(); return <div className={`month-day${outside ? " outside" : ""}${iso(day) === iso(today) ? " today" : ""}`} key={iso(day)}><time>{day.getDate()}</time>{departures.map((stay) => <Link className="month-departure" href={`/pobyty/${stay.id}`} key={stay.id}>Odjezd ráno</Link>)}</div>; })}{laidOut.map(({ stay, lane, startColumn, endColumn }) => <Link className={`month-stay-bar ${stay.source}`} style={{ gridColumn: `${startColumn} / ${endColumn}`, gridRow: lane + 2 }} href={`/pobyty/${stay.id}`} key={`${stay.id}-${weekStart}`}><strong>{stay.source === "airbnb" ? "Airbnb" : "Pobyt"}</strong><span>{stay.guests} hosté</span></Link>)}</div>;
}
