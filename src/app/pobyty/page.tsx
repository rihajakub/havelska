...
export default async function StaysPage() {
  ...
  const today = fromIso(iso(new Date())); const firstUpcoming = stays.find((stay) => stay.checkOut >= iso(today));
  const start = monday(firstUpcoming ? fromIso(firstUpcoming.checkIn) : today); const weeks = Array.from({ length: 8 }, (_, index) => addDays(start, index * 7));
  return <>
    ...
    <section className="calendar-section" aria-labelledby="calendar-title"><div className="calendar-title"><div><span className="eyebrow">Přehled rezervací</span><h2 id="calendar-title">Týdenní kalendář</h2></div><p>Den odjezdu je opět volný.</p></div><div className="calendar-scroll"><div className="week-calendar"><div className="calendar-corner">Týden</div>{Array.from({ length: 7 }, (_, index) => <div className="calendar-weekday" key={index}>{weekdayFormat.format(addDays(start, index))}</div>)}{weeks.flatMap((weekStart) => [<div className="calendar-week-label" key={`${iso(weekStart)}-label`}>{dayFormat.format(weekStart)} – {dayFormat.format(addDays(weekStart, 6))}</div>, ...Array.from({ length: 7 }, (_, index) => { const day = addDays(weekStart, index); return <div className={`calendar-day${iso(day) === iso(today) ? " today" : ""}`} key={iso(day)}><time dateTime={iso(day)}>{dayFormat.format(day)}</time>{staysOnDay(stays, day).map((stay) => <Link className={`calendar-stay ${stay.source}`} href="#stay-list" key={stay.id} title={`${stay.source === "airbnb" ? "Airbnb rezervace" : "Ruční pobyt"}: ${stayRange(stay)}, ${stay.guests} hosté, připravit pro ${stay.preparationGuests}`}><strong>{stay.source === "airbnb" ? "Airbnb" : "Ruční pobyt"}</strong><span>{stayRange(stay)}</span><small>{stay.guests} hosté · odjezd {dayFormat.format(fromIso(stay.checkOut))}</small></Link>)}</div>; })])}</div></div></section>
    <div className="calendar-title stays-title"><div><span className="eyebrow">Detailní přehled</span><h2 id="stay-list">Seznam pobytů</h2></div></div>
    {stays.length === 0 ? <section className="empty"><CalendarEmpty/><h2>Zatím žádný pobyt</h2><p>Přidej první rezervaci. Import Airbnb iCal doplníme po ověření skutečného exportu.</p><Link className="button" href="/pobyty/novy">Přidat ručně</Link></section> : <div className="stay-list">{stays.map((stay) => <article className="stay-card" key={stay.id}><div className="date-tile"><strong>{fromIso(stay.checkIn).getDate()}</strong><span>{new Intl.DateTimeFormat("cs-CZ", { month: "short" }).format(fromIso(stay.checkIn))}</span></div><div><span className="eyebrow">{stay.source === "manual" ? "Ručně" : "Airbnb"}</span><h2>{detailFormat.format(fromIso(stay.checkIn))} – {detailFormat.format(fromIso(stay.checkOut))}</h2><p>{stay.guests} hosté · připravit pro {stay.preparationGuests}{stay.note ? ` · ${stay.note}` : ""}</p><form action={updateGuests} className="guest-count-form"><input type="hidden" name="stayId" value={stay.id}/><label><span>Počet hostů</span><select name="guests" defaultValue={stay.guests}>{[1, 2, 3, 4].map((count) => <option key={count} value={count}>{count}</option>)}</select></label><button className="button secondary" type="submit">Uložit</button>{stay.guestCountManuallySet && <small>nastaveno ručně</small>}</form></div></article>)}</div>}
  </>;
}
