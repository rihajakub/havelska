...
export default async function StayDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const registrations = await getCheckInRegistrations(); const registration = registrations.find((item) => item.stay.id === stay.id);
  ...
  return <>
    ...
    <div className="stay-detail-grid">
      ...
      <section className="detail-card detail-wide"><span className="eyebrow">Cizinecká policie</span><h2>UbyPort a domovní kniha</h2><p>{registration?.registration.reportedAt ? "Hlášení bylo označeno jako odeslané do UbyPortu." : registration?.registration.submittedAt ? "Host údaje vyplnil — přepiš je do UbyPortu a potvrď odeslání." : registration ? "Čekáme na vyplnění formuláře hostem." : "Pro tento pobyt ještě nebyl vytvořen check-in odkaz."}</p><Link className="button" href="/cizinecka-policie">Otevřít hlášení</Link></section>
    </div>
  </>;
}
